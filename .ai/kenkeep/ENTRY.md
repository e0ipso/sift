...
- Load [`cross-card/`](nodes/cross-card/index.md) for more information on the rules sift-drain and sift-prime must classify alike — what a roadmap row is and what a ticket ID is — and how every copy is inventoried and guarded; read when touching either card's row reader or ID check.
