...
description: >-
  src/skills/sift-{init,drain,prime} is the source of the sift cards;
...
---
The three sift skill cards live at `src/skills/sift-init/`, `src/skills/sift-drain/` and `src/skills/sift-prime/`. Each holds its own `SKILL.md`, `scripts/` and `references/`. This repository is the source of the cards rather than a consumer of them.

The harness-visible installation at `.claude/skills/sift-init`, `.claude/skills/sift-drain` and `.claude/skills/sift-prime` consists of symlinks pointing at those `src/skills/` directories, and the symlinks are tracked in git. A path reached through `.claude/skills/` and the corresponding path under `src/skills/` are therefore the same file. The non-sift cards in `.claude/skills/` symlink to `.agents/skills/` instead.

The practical consequence is that running a card here exercises the working tree, not an installed copy, and an edit under `src/skills/` is immediately live for any agent that loads the card.

When changing this, note that a generic "do not edit the running skill's own files" instruction does not mean "do not edit `src/skills/`" in this repository — the card *is* the product here. Decide that case deliberately rather than by reflex.
