...
`scripts/` next to SKILL.md holds the deterministic helpers called instead of parsing markdown by eye: `wave-status.sh` (per-wave done/remaining and the current wave's remaining tickets — the orchestrator's wave load), `next-ticket.sh` (a cluster-widening helper, not the drain loop), `ticket-check.sh` (rule-9 consistency, non-zero on violation), `drain-log.sh` (per-dispatch runtime and idle attribution; the only writer of `RUNLOG.md`), `reserve-ids.sh` (persistent ID allocation), `prepare-worktree.sh` (coordinator-owned branch setup), and `list-labels.sh` / `tickets-by-label.sh` (label queries), over a shared `lib.sh`. They find the project root by walking up from `$PWD` for a `.ai/sift/` directory and read the prefix from `.ai/sift/config/config.yaml`; `SIFT_ROOT` and `SIFT_PREFIX` override.
