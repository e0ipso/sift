...
kk_confidence: high
---
The cookbook tests execute operation bodies extracted from `src/operations/sift.sh` by `# BEGIN`/`# END` markers (and README.md's normative blocks by anchor line), and the coupling is the point: a renamed marker or reworded anchor makes the extractor return nothing so the test fails loudly. It only fails loudly when the assertion can see it. An empty extraction runs nothing, prints nothing and exits 0 — which is exactly what `assert_eq "" "$R_OUT"`, an `assert_not_contains`, or a `grep -c` expected to be zero asserts. Every negative-only case stays green with its recipe deleted.
...
Measure it the same way: empty every marked body in a copy of `src/operations/sift.sh`, re-run, and require every case in the file to report a failing assertion. `tests/lib/recipes.sh` resolves `REPO_ROOT` from its own location, so a copy of the tree — `src`, `tests`, `README.md`, `schemas/` — is a complete harness for that measurement, and the real script is never touched.
