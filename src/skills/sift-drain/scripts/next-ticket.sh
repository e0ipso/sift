...
#
# Reads ROADMAP.md in wave order and reports the first unstruck, dispatchable
# ticket plus selected front matter. The lookup state is `result: found|none`;
...
while IFS=$'\t' read -r wave order id struck _title; do
  ...
  case "$file" in
    */archive/*)
      # An archived unstruck ticket is inconsistent, not dispatchable.
...
# --- The dispatch group ------------------------------------------------------
# Grouping runs after lead selection, preserving default row order and output.
...
if [ -n "$LEAD_CLUSTER" ]; then
  ...
  while IFS=$'\t' read -r wave _order id struck _title; do
    ...
    # Stop at the first bound breach to preserve roadmap order.
